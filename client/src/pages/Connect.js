...
export default function Connect({ onConnected }) {
  ...
  useEffect(() => {
    ...
    if (params.get('error')) { setError('Google sign-in failed. Please try again.'); window.history.replaceState({}, '', '/'); }
    if (window.location.hash === '#select-property') {
  ...
  async function loadProps() {
    ...
    } catch { setError('Could not load GA4 properties. Make sure Google Analytics Data API is enabled.'); }
    setLoading(false);
  }
  ...
  async function handleSelect() {
    ...
    setLoading(true);
    try {
      await api.post('/properties/select', { propertyId: selected, propertyName: selName, siteUrl });
      onConnected();
      ...
      <div style={styles.card}>
        ...
        {step === 'login' && (
          <>
            ...
            <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--faint)', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '.5px' }}>What you'll see after connecting</div>
            {['Traffic overview + trend comparison', 'AI traffic — ChatGPT, Perplexity, Claude, Gemini', 'Social media platform breakdown', 'Referral domains + conversion rates', 'Shopify App Store traffic & keyword installs', 'Return visitor analysis', 'SEO rankings from Search Console', 'Competitor keyword analysis'].map((f, i) => (
          ...
          <>
            ...
            {loading ? <div className="loading"><div className="spinner" />Loading your properties…</div> : (
              <>
                ...
                <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '11px' }} disabled={!selected || loading} onClick={handleSelect}>
